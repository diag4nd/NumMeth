...
long double midDerivSecond(int i, long double *x, long double *f)
{
  long double h = x[1] - x[0];
  if (i == 0)
  {
    return (2*f[i] - 5*f[i+1] + 4*f[i+2] - f[i+3])/(h*h);
  }
  else if (i == N-1)
  {
    return (2*f[i] - 5*f[i-1] + 4*f[i-2] - f[i-3])/(h*h);
...
int main()
{
    ...
    out.open("midDeriveSecond.txt");
    if (out.is_open())
    {
        for (int i=0; i<N; i++)
        {
          df[i] = midDerivSecond(i, x, f);
          out << x[i] << '\t' << df[i] << std::endl;
        } 
    }
    out.close();
